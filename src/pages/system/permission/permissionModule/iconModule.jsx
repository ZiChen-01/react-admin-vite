...
const IconModule = forwardRef((props, ref) => {
    ...
    // 获取icon名称，目前尚未有时间研究别的优良方案，此获取icon-name方法为无奈之举，后期如有时间，将会改良此组件
    const chooseIcon = (item) => {
        // 提取render函数名称、函数名称一般情况为icon名称，但结尾包含数字、具体原因得查看antd源码
        let renderName = ""
        for (let i = 0; i < item.render.name.length; i++) { // 去除函数名称结尾数字
            if (isNaN(item.render.name[i])) {
                renderName += item.render.name[i];
            }
        }
        let iconName = renderName || item.render.displayName || item.displayName
